...
**Last updated:** 2026-04-16 (Session close — ARCH-V2 spec v1.1 LOCKED on disk (b94719c); Coach Voice Seed v0.2 ready, held in chat; four-layer KB ADR finalised; PASS1-SCHEMA is next P0.)
...
- **UI-WIRE-1 — COMPLETE (2026-04-15).** ClaudeProvider added (`claude-sonnet-4-6`). `runFullPipeline(buffer, riderNote?)` wired in runner.ts — Stages 0–11 chained. API route wired: `USE_MOCK = false`, gated response handling, real pipeline path. Five build fixes: Turbopack import resolution (`.js` extensions stripped from all pipeline imports, 13 files), `serverExternalPackages` for ffmpeg/ffprobe native modules, `export const runtime = 'nodejs'` on API route, JPEG frame detection from buffer magic bytes (was hardcoded PNG), model ID corrected to `claude-sonnet-4-6`. Key files: `pipeline/model-provider.ts` (ClaudeProvider), `pipeline/runner.ts` (runFullPipeline), `app/api/analyze/route.ts` (real pipeline wired), `next.config.ts` (serverExternalPackages).
- **UI-TEST-1 — PARTIAL (3/8 clips, 2026-04-15).** Mark Crash: coaching produced, mechanism plausible, trust envelope working. 9.3 min. Long Hill: no false-fail on clean POV, but "coaching unavailable" due to Stage 9 bug. Colin Hill: initially missed bail entirely; Stage 9 bug found and fixed; post-fix produces coaching ("Dynamic Stability in the Rutted Climb"). 7.4 min. Regression investigation: T1 used GPT-4o + no Stage 0; UI path uses Claude Sonnet 4.6 + Stage 0 + trust envelope — three simultaneous changes. Trust envelope bypass test confirmed Stage 0 was NOT the primary cause — Stage 9 decision logic was the real bug. 5 clips remain. Scoring sheet v2 committed (`19f717e`).
- **Stage 9 coaching decision fix — COMMITTED (96b3846, 2026-04-15).** Two bugs fixed. (1) Normalizer: `!output.coaching_required` alone triggered destructive wipe of `primary_focus`, `secondary_points`, `excluded_factors`, `safety_flags`. Fix: `isClean` now requires all three conditions — structural clean AND no failure type AND model says no coaching. (2) Prompt Rule 8 replaced: was "clean completions = no coaching required." New rule distinguishes truly clean clips from clean-with-instability. Foot dabs, balance corrections, traction loss, near-miss, high-risk terrain all warrant `coaching_required: true`. For clean-but-coachable cases: optimisation/improvement framing, not failure-correction. User prompt context note updated to match.
- **Architecture discussion — KB retrieval + coaching framework (2026-04-15).** UI-TEST-1 reveals pipeline produces technically careful but not coach-sharp output. Standing on pegs not mentioned for steep climb. Core problem: LLM re-derives static KB facts (steep dusty rut → weight forward, active balance, momentum) every call — these are already in the KB. Proposed direction: (1) three-part coaching framework (Situation → Breakdown → Correction); (2) KB as deterministic retrieval — after perception identifies terrain/features, query KB for known constraints, risks, required techniques and common errors; (3) target architecture: Perceive (1 vision call) → KB lookup (instant) → Reason → Coach. Key challenges: KB entries are markdown prose, not queryable key-value pairs — retrieval layer is real engineering; perception dominates latency regardless of KB speed (vision API + 16 frames = 3–4 min regardless); bridge from Stage 4 detection → right KB entry → right coaching rule is the hard part. **Recommended sequence:** (1) Wire KB retrieval into existing Stage 10 placeholder — smallest test of highest-value hypothesis; (2) Parallelise S1–S4 with Promise.all — latency win, no architecture change; (3) If KB improves coaching quality: design condensed coaching framework; (4) If not: problem is elsewhere and redesign is premature.
- **ARCH-V2 implementation — PARTIALLY BUILT (2026-04-15, commit 0987fa6).** Built: `lib/retrieval/` — deterministic context pack (~1154 lines, `buildContextPack`, `fromStage4` adapter, `types.ts` with `ContextPack`/`PerceptionInput`/artifact interfaces); `lib/validation/outputValidator.ts` — Amendments 1/4/5/6 enforced; `lib/safety/enduroSafetyRules.ts` — 11 safety rules; `lib/benchmark/benchmarkHarness.ts` — harness with 3 golden-path fixtures all passing. NOT BUILT: `docs/arch-v2-spec.md` (parent spec being drafted in chat); Pass 1 combined vision call; Pass 1 output schema; Pass 2 LLM call (only output schema stub exists at `lib/pass2/prompt.md`). Amendments 2 (KB sweep), 3 (retrieval ranking), 7 (quality bar) status unresolved. `arch-v2/src/` JS ports deleted — dead code, zero references.
- **Four-layer KB architecture — DECIDED (2026-04-16).** Canonical KB (authored markdown, source of truth) / Research Corpus (external ingest via Track 2 agent, not product truth) / Proposal Layer (human-gated promotion gate) / Runtime Retrieval Pack (`buildContextPack` output, Canonical KB only). Phase 3 retrieval is deterministic markdown-driven from structured Pass 1 output — embedding/vector retrieval deferred to Track 2 research agent as a separate workstream, does not block Track 1. Coach Voice Seed Spec required before any Track 2 ingest. ADR at `docs/kb-architecture.md`.
- **Known issues (post UI-TEST-1).** Latency: 7–9 min per clip — S1–S4 parallelisation is first win; real unlock requires fewer/combined vision calls or local CV. KB not connected: Stage 10 coaching is generic, domain-specific rules (stand on pegs, weight through pegs on steep climb) not surfaced. POV clean clips: produce no coaching — need coaching paths for non-failure clips on line choice, speed, terrain reading. Stage 0 may over-degrade: Colin Hill 3rd-person gets `degrade` despite 0.97 camera confidence — threshold tuning needed. Stage 3 latency: ~1m30s for intent detection alone.
- **Strategic decision (2026-04-15): S0–S11 is now benchmark/debug/evaluation infrastructure only — not the product path.** It has served its purpose: proving stage contracts, reasoning chains, observability gating, and trust envelope concepts. But it is too slow (7–9 min/clip) and too generic (no KB, no bike data) to be the product. Testing more clips through a system missing its core differentiators tests generic model capability, not RideMind's product. The product path must be rebuilt around a condensed architecture with KB integration from day one. S0–S11 is retained, not deleted — still runnable for regression testing and detailed diagnosis.
- **ARCH-V2 — target architecture (three passes + KB retrieval, target under 90 sec).** Pass 1 (Perception): one vision call replacing S0–S4 — camera, observability, terrain, features, intent, key events, visible instability. Single structured output. Target: 30–45 sec. KB Retrieval (deterministic, no LLM): maps perception output → terrain KB, feature KB, machine KB; assembles coaching context pack with constraints, common errors, required techniques, machine-specific effects. Target: under 1 sec. Pass 2 (Coaching): one LLM call — perception output + KB pack + rider note → Situation / Breakdown / Correction. Target: 15–30 sec. Safety: rules-based on output, no separate LLM call. Key challenges: KB entries are markdown prose not queryable KV pairs — retrieval layer is real engineering; perception dominates latency regardless of KB speed; bridge from structured detection → right KB entry → right coaching rule is the hard part. **Spec before code — define perception output schema, KB retrieval interface, and coaching output schema before any implementation.**
- **ARCH-V2 spec v1.1 — LOCKED (commit b94719c, 2026-04-16) at `docs/arch-v2-spec.md` (748 lines).** All 7 amendments folded. Two architectural details finalised in v1.1: §11 empty-ContextPack rule returns `analysis_unavailable_low_context` structured failure (not `coaching_required: false`); §7.5 schema versioning contract — PerceptionOutput / Pass2Output / PipelineOutput each carry `schema_version` once producing stage is built. Pass 2 schema redesign explicitly **DEFERRED** with revisit trigger after first 8-clip benchmark. Amendment 3 (retrieval ranking) fully documented with scoring composition. Amendment 5 confirmed enforced against `perception.events` (currently unreachable; live when Pass 1 lands). Failure mode named: **chat-lock is not repo-lock** — spec lock requires committed file path + commit hash.
- **Coach Voice Seed v0.2 — READY, held in Claude.ai chat.** Content not yet on disk. To land next session as `docs/coach-voice-seed-v1.md`. Gates all Track 2 research agent ingest. v0.2 addressed: drill rule locked (0-1 default, 2 with justification, 3+ never), §2.1 cross-reference with ARCH-V2, §8.7 rider note conflict voice, §9.3 crash-register example, §3.5/§3.8/§5.4/§7.5/§7.7 sharpened, §11.2/§11.6 resolved.
- **Next actions (P0 order):** (1) ✓ ARCH-V2-SPEC — COMPLETE (commit b94719c). (2) PASS1-SCHEMA — design Pass 1 output schema; replaces `fromStage4()` adapter; blocks Pass 1 implementation, Pass 2 design, and full ARCH-V2 benchmark runs. (3) PASS1-IMPL — single combined vision call replacing S0–S4, target 30–45 sec; blocked on PASS1-SCHEMA. (4) PASS2-PROMPT — author full Pass 2 prompt against existing output schema in `lib/pass2/prompt.md`; blocked on PASS1-SCHEMA. (5) PASS2-IMPL — Pass 2 LLM call implementation; blocked on PASS2-PROMPT. (6) AMEND-2-RESOLVE — decide enforce/defer/drop for Amendment 2 (KB corpus sweep); Amendments 3 and 7 folded into ARCH-V2 v1.1. (7) BENCHMARK-ARCH-V2 — full 8-clip run, compare quality + latency vs S0–S11 baseline.
