...
**Last updated:** 2026-04-16 (Doc hygiene patch — repo state reconciled; Track 1 / Track 2 separation locked; Pass 1 design is next P0 after spec.)
...
- **Next actions (P0 order):** (1) ARCH-V2-SPEC — write `docs/arch-v2-spec.md` as parent for ARCH-V2-AMENDMENTS_1.md, reconstructed from existing code. In progress in chat — separate commit. (2) PASS1-SCHEMA — design Pass 1 output schema; replaces `fromStage4()` adapter; blocks Pass 1 implementation, Pass 2 design, and full ARCH-V2 benchmark runs. (3) PASS1-IMPL — single combined vision call replacing S0–S4, target 30–45 sec; blocked on PASS1-SCHEMA. (4) PASS2-PROMPT — author full Pass 2 prompt against existing output schema in `lib/pass2/prompt.md`; blocked on PASS1-SCHEMA. (5) PASS2-IMPL — Pass 2 LLM call implementation; blocked on PASS2-PROMPT. (6) AMEND-2-3-7-RESOLVE — decide enforce/defer/drop for Amendments 2, 3, 7; lock before ARCH-V2.1 final lock. (7) BENCHMARK-ARCH-V2 — full 8-clip run, compare quality + latency vs S0–S11 baseline.
