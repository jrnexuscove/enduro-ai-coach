...
**Last updated:** 2026-04-15 (UI-WIRE-1 complete; UI-TEST-1 partial 3/8 clips; Stage 9 coaching decision fix; architecture discussion — KB retrieval + coaching framework path forward)
...
- **Next actions (P0 order):** (1) ARCH-V2 spec — perception output schema, KB retrieval interface, coaching output schema, safety rules — locked on paper before any code; (2) KB retrieval layer — function mapping perception output to KB entries, reusable regardless of architecture; (3) Pass 1 implementation — single combined vision call replacing S1–S4, biggest latency win; (4) Pass 2 implementation — coaching synthesis with KB pack + rider note; (5) Benchmark ARCH-V2 vs S0–S11 — same clips, compare quality + latency; S0–S11 becomes the reference baseline.
