...
function loadKnowledgeBase(): string {
  ...
  // Extracts only the sections most useful for real-time analysis, discarding drills,
  // cross-references, terrain variations, and expert insights to save tokens.
  function extractSections(content: string): string {
    const KEEP = ["OVERVIEW", "DIAGNOSTIC CUES", "COACHING CUES", "COMMON ERRORS", "ERROR PATTERNS"];
    // Split on level-2 headings using a lookahead so each heading stays with its section body
    ...
    for (const part of parts) {
      ...
      if (!headingMatch) {
        // Frontmatter + title block — keep only the # Title line
  ...
  // Reads .md files from a subdirectory, optionally filtered by keywords in the filename.
  // Returns an array of extracted-section strings, one entry per file.
  ...
  // Load file extracts in priority order so the cap drops least-relevant content first:
  // 1. All hill and technical terrain files (highest relevance for hill climbing test)
  // 2. Biomechanics files covering standing or body position
  // 3. Error pattern files covering body position errors
  // 4. Bike control files covering throttle and clutch technique
  ...
  // Apply the character cap, keeping whole file extracts in priority order.
  // Dropping whole files is better than slicing mid-section.
...
async function analyseRiding(
  ...
  // Step 1: Visual observation — no KB, pure literal description of what is seen
  console.log("Step 1: Visual observation (no KB)...\n");

  const imageMessages = framePaths.map((fp) => ({
    type: "image_url" as const,
    image_url: { url: "data:image/jpeg;base64," + fileToBase64(fp), detail: "high" as const },
  }));
  ...
  const visualObs = visualResponse.choices[0].message.content || "";
  console.log("Visual observation complete.\n");
  console.log("\n--- RAW VISUAL OBSERVATION ---");
  console.log(visualObs);
  console.log("--- END RAW VISUAL ---\n");

  // Step 2: Audio observation — no KB, pure literal description of what is heard
  ...
  // Step 3: Scenario classification — no KB, uses both raw observations to label the scenario
  ...
  const scenarioClass = classificationResponse.choices[0].message.content || "";
  console.log("Scenario classification complete.\n");
  console.log(scenarioClass + "\n");

  // Step 4: Coaching — KB is introduced here for the first time
  // Receives the raw observations and classification; uses KB to interpret and advise
  ...
  return coachingResponse.choices[0].message.content || "No response received";
}

async function main() {
  const videoPath = process.argv[2];
  if (!videoPath) { console.error("Usage: npx tsx scripts/test-coaching-kb.ts path/to/video.mp4"); process.exit(1); }
  ...
  console.log("=== RideMind - Knowledge Base Enhanced Coaching Test ===\n");
  ...
  // Save output to a timestamped file for comparison with the baseline run
  const timestamp = new Date()
    .toISOString()
    .replace(/:/g, "-")
    .replace(/\..+/, "");
  const outputPath = path.join("scripts", `coaching-output-kb-${timestamp}.txt`);
  const outputContent = [
    `RideMind - Knowledge Base Enhanced Coaching Output`,
